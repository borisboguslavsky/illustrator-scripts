 ...
 *
 * Based on the Random Layer Scale Functionality from the Randomill Illustrator Plugin
 * https://randomill.com/
 *
...
dialog.text = "Random Scale - randomill.com"
...
yInputGroup.alignChildren = ["fill", "center"]
...
var uniformScale = optionsPanel.add("checkbox")
uniformScale.text = "Preserve Aspect Ratio (Uniform Scale)"
uniformScale.value = true
uniformScale.onClick = function () {
	if (this.value) {
		yInputGroup.visible = false
		xMinLabel.text = "Min \u0025"
		xMaxLabel.text = "Max \u0025"
		xStepLabel.text = "Step \u0025"
		yMinField.enabled = false
		yMaxField.enabled = false
		yStepField.enabled = false
	} else {
		yInputGroup.visible = true
		xMinLabel.text = "X Min \u0025"
		xMaxLabel.text = "X Max \u0025"
		xStepLabel.text = "X Step \u0025"
		yMinField.enabled = true
		yMaxField.enabled = true
		yStepField.enabled = true
	}
}
...
yInputGroup.visible = !uniformScale.value
xMinLabel.text = uniformScale.value ? "Min \u0025" : "X Min \u0025"
xMaxLabel.text = uniformScale.value ? "Max \u0025" : "X Max \u0025"
xStepLabel.text = uniformScale.value ? "Step \u0025" : "X Step \u0025"
yMinField.enabled = !uniformScale.value
yMaxField.enabled = !uniformScale.value
yStepField.enabled = !uniformScale.value
...
function randomizeScale(objects, uniform, separateGroups, separateCompoundPaths) {
	...
	for (var i = 0; i < objects.length; i++) {
		var obj = objects[i]
		var xScale = randomValueBetween(xMin, xMax, xStep)
		var yScale = randomValueBetween(yMin, yMax, yStep)

		if (uniform) yScale = xScale

		if (obj.typename == "CompoundPathItem" && separateCompoundPaths) {
			randomizeScale(obj.pathItems, uniform, separateGroups, separateCompoundPaths)
		} else if (obj.typename == "GroupItem" && separateGroups) {
			for (var j = 0; j < groupItemTypes.length; j++) {
				randomizeScale(obj[groupItemTypes[j]], uniform, separateGroups, separateCompoundPaths)
			}
		} else {
