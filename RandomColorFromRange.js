...
// dialog window
var dialog = new Window("dialog", "Random Color (Range) \u00A9 randomill.com", undefined)
...
// mode tabs
var modeTabPanel = dialog.add("tabbedpanel")
modeTabPanel.spacing = 10
modeTabPanel.margins = 10
modeTabPanel.alignment = ["fill", "top"]
...
hslFields.spacing = 10
hslFields.margins = 10
...
rgbFields.spacing = 10
rgbFields.margins = 10
...
cmykFields.spacing = 10
cmykFields.margins = 10
...
var fillOrStrokeRadios = optionsPanel.add("group")
var applyTo = fillOrStrokeRadios.add("statictext")
applyTo.text = "Apply to:"
fillOrStrokeRadios.orientation = "row"
fillOrStrokeRadios.alignment = ["fill", "top"]
fillOrStrokeRadios.alignChildren = ["left", "top"]
fillOrStrokeRadios.spacing = 10
fillOrStrokeRadios.margins = 0
var fillsRadio = fillOrStrokeRadios.add("radiobutton")
fillsRadio.text = "Fills"
fillsRadio.value = true
var strokesRadio = fillOrStrokeRadios.add("radiobutton")
strokesRadio.text = "Strokes"
...
footerButtons.alignChildren = ["fill", "fill"]
footerButtons.margins = [0, 0, 0, 0]
...
upgadeButton.onClick = function () {
	try {
		var URL = new File(Folder.temp + "/RandomillSite.html")
		URL.open("w")
		URL.writeln(
			'<html><HEAD><meta HTTP-EQUIV="REFRESH" content="0; url=https://randomill.com/"></HEAD></HTML>'
		)
		URL.close()
		URL.execute()
	} catch (e) {
...
var close = footerButtons.add("button", undefined, "Close")
close.onClick = function () {
	dialog.close()
}
var apply = footerButtons.add("button", undefined, "Apply")
apply.onClick = function () {
	try {
		...
		// ensure some swatches are selected
		var swatches = app.activeDocument.swatches.getSelected()
		if (!swatches || swatches.length === 0) {
			throw new Error("No currently selected swatches. Select objects first, then select swatches.")
		}

		randomizeColorsFromRange(modeTabPanel.selection.text, objects)
		app.redraw()
	} catch (err) {
		alert("Error: \n" + err.message)
	}
...
function randomizeColorsFromRange(colorMode, objects) {
	...
	// temporary placeholder for the generator function
	// changes depending on the color mode selected
	...
	if (colorMode === "HSL") {
		...
		throw new Error("Invalid color mode selected: " + colorMode)
	}

	// establish the fill method
	var fillOrStroke = fillsRadio.value === true ? "fillColor" : "strokeColor"
	...
	for (i = 0; i < objects.length; i++) {
		...
		setColorProperty(item, generatorFn, randomizeWithinGroupsCheckbox.value === true, fillOrStroke)
	}
