 ...
 *
 * Based on the Random Rotation Functionality from the Randomill Illustrator Plugin
 * https://randomill.com/
 *
...
dialog.text = "Random Rotation - randomill.com"
...
// min field
var minGroup = inputGroup.add("group")
minGroup.orientation = "column"
minGroup.alignChildren = ["fill", "center"]
minGroup.spacing = 10
minGroup.margins = 0
minGroup.alignment = ["left", "fill"]
var minLabel = minGroup.add("statictext")
minLabel.text = "Min \u00B0 [0-360]"
var minField = minGroup.add("edittext")
minField.text = "0.00"
minField.preferredSize.width = 95
minField.onChanging = sanitizeInput

// max field
var maxGroup = inputGroup.add("group")
maxGroup.orientation = "column"
maxGroup.alignChildren = ["fill", "center"]
maxGroup.spacing = 10
maxGroup.margins = 0
maxGroup.alignment = ["left", "fill"]
var maxLabel = maxGroup.add("statictext")
maxLabel.text = "Max \u00B0 [0-360]"
var maxField = maxGroup.add("edittext")
maxField.text = "360.00"
maxField.preferredSize.width = 95
maxField.onChanging = sanitizeInput

// step field
var stepGroup = inputGroup.add("group")
stepGroup.orientation = "column"
stepGroup.alignChildren = ["fill", "center"]
stepGroup.spacing = 10
stepGroup.margins = 0
stepGroup.alignment = ["left", "fill"]
var stepLabel = stepGroup.add("statictext")
stepLabel.text = "Step \u00B0 [0-360]"
var stepField = stepGroup.add("edittext")
stepField.text = "1.00"
stepField.preferredSize.width = 95
stepField.onChanging = sanitizeInput
...
var randomizeWithinCompoundPathsCheckbox = optionsPanel.add("checkbox")
randomizeWithinCompoundPathsCheckbox.text = "Randomize Within Compound Paths"
randomizeWithinCompoundPathsCheckbox.value = false
...
apply.onClick = function () {
	try {
		...
		randomizeRotation(
			...
			randomizeWithinCompoundPathsCheckbox.value
		)
		app.redraw()
	} catch (err) {
		alert("Error: \n" + err.message)
...
function randomizeRotation(objects, separateGroups, separateCompoundPaths) {
	var min = parseFloat(minField.text)
	var max = parseFloat(maxField.text)
	var step = parseFloat(stepField.text)
	...
	for (var i = 0; i < objects.length; i++) {
		...
		if (obj.typename == "CompoundPathItem" && separateCompoundPaths) {
			randomizeRotation(obj.pathItems, separateGroups, separateCompoundPaths)
		} else if (obj.typename == "GroupItem" && separateGroups) {
			for (var j = 0; j < groupItemTypes.length; j++) {
				randomizeRotation(obj[groupItemTypes[j]], separateGroups, separateCompoundPaths)
			}
		} else {
			var rotation = randomValueBetween(min, max, step)
			obj.rotate(rotation)
		}
	}
}
