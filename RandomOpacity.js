 ...
 *
 * Based on the Random Opacity Functionality from the Randomill Illustrator Plugin
 * https://randomill.com/
 *
...
maxLabel.text = "Max \u0025 [0-100]"
var maxField = maxGroup.add("edittext")
maxField.text = "100.00"
...
apply.onClick = function () {
	try {
		...
		randomizeOpacity(objects, randomizeWithinGroupsCheckbox.value)
		app.redraw()
	} catch (err) {
		alert("Error: \n" + err.message)
	}
...
function randomizeOpacity(objects, separateGroups) {
	...
	for (var i = 0; i < objects.length; i++) {
		var obj = objects[i]
		if (obj.typename == "GroupItem" && separateGroups) {
			for (var j = 0; j < groupItemTypes.length; j++) {
				randomizeOpacity(obj[groupItemTypes[j]], separateGroups)
			}
		} else {
			var opacity = randomValueBetween(min, max, step)
			obj.opacity = opacity
		}
	}
}
